# Note in VectorDB class modify variables w.r.t your Database credentials
...
class VectorDB:
    ...
    def _compute_attention_scales(self, layer_idx: int) -> np.ndarray:
        ...
        :rtype: np.ndarray
        """
        all_heads = ", ".join(f"head{head:02}" for head in range(16))
        head_embs = self._fetch(f"SELECT {all_heads} FROM attention WHERE layer_index = %s;", (layer_idx,))

        all_heads_avg = ", ".join(f"AVG(head{head:02} {DistanceMetric.COSINE.value} %s) " for head in range(16))
        dist_sql = f"SELECT {all_heads_avg} FROM attention WHERE layer_index = %s;"
        pairwise_distances = np.zeros((len(head_embs), 16))
        ...
        all_heads_avg = ", ".join(f"AVG(head{head:02} {DistanceMetric.EUCLIDEAN.value} %(zero)s) " for head in range(16))
        dist_sql = f"SELECT {all_heads_avg} FROM attention WHERE layer_index = %(layer)s;"
        norms = np.array(self._fetch(dist_sql, dict(zero=np.zeros(64), layer=layer_idx))[0])
    ...
    def _compute_cut_standard_scales(self) -> np.ndarray:
        ...
        :rtype: np.ndarray
        """
        all_segments = ", ".join(f"segment{seg:02}" for seg in range(16))
        seg_embs = self._fetch(f"SELECT {all_segments} FROM cut_standard;")

        all_segs_avg = ", ".join(f"AVG(segment{seg:02} {DistanceMetric.COSINE.value} %s)" for seg in range(16))
        dist_sql = f"SELECT {all_segs_avg} FROM cut_standard;"

        pairwise_distances = np.zeros((len(seg_embs), 16))
        ...
        all_segs_avg = ", ".join(f"AVG(segment{seg:02} {DistanceMetric.EUCLIDEAN.value} %(zero)s)" for seg in range(16))
        dist_sql = f"SELECT {all_segs_avg} FROM cut_standard;"
        norms = np.array(self._fetch(dist_sql, dict(zero=np.zeros(64)))[0])
    ...
    def _split_embedding(embedding: Embedding) -> list[Embedding]:
        ...
        """
        chunk_size, rem = divmod(len(embedding), 16)
    ...
    def _add_cut_embeddings(self, article_id: int, embedding: Embedding) -> None:
        ...
        """
        all_segments = ", ".join(f"segment{seg:02}" for seg in range(16))
        all_values = ", ".join("%s" for _ in range(17))
    ...
    def _add_attention_embeddings(self, article_id: int, layer_idx: int, embeddings: list[Embedding]) -> None:
        ...
        """
        all_heads = ", ".join(f"head{head:02}" for head in range(16))
        all_values = ", ".join("%s" for _ in range(18))
    ...
    def _add_article(self, article_emb: ArticleEmbeddings) -> None:
        ...
        sql = "INSERT INTO articles (label) VALUES (%s) RETURNING id;"
        article_id: int = self._fetch(sql, (article.text,))[0][0]
    ...
    def standard_search(self, emb: FullEmbeddings, n: int = 16) -> list[tuple[float, Article]]:
        """
        ...
        for distance, text in res[:n]:
            articles.append((distance, Article(text)))
    ...
    def attention_search(
        ...
        for i, embedding in enumerate(embeddings):
            ...
            articles: list[tuple[float, Article]] = []
            for distance, text in head_res[:n]:
                articles.append((distance, Article(text)))
    ...
    def cut_standard_search(self, emb: FullEmbeddings, n: int = 16) -> list[list[tuple[float, Article]]]:
        """
        ...
        for i, embedding in enumerate(chunks):
            ...
            articles: list[tuple[float, Article]] = []
            for distance, text in seg_res[:n]:
                articles.append((distance, Article(text)))
